...
namespace DoggyChef
{
    ...
    public class Match
    {
        // 이 매치에 속한 보석들의 그리드 좌표 목록.
        public List<Vector3Int> MatchingGem = new();

        // 매치가 발생한 기준 셀 (보너스 젬을 여기에 생성합니다).
        public Vector3Int OriginPoint;

        // 삭제 애니메이션 타이머 (1.0f를 초과하면 실제 파괴).
        public float DeletionTimer = 0f;

        // 조건 충족 시 생성될 보너스 젬 프리팹 (없으면 null).
        public BonusGem SpawnedBonus = null;

        // true 이면 타이머 없이 즉시 파괴 (보너스 젬 강제 삭제 모드).
