...
public class GoldGauge : MonoBehaviour
{
    ...
    private readonly int coinUnit = 100;           // 100에 1코인
    private readonly int maxCoinsPerBurst = 20;    // 한 번에 최대 코인 수 (성능 보호)
    ...
    void OnEnable()
    {
        ...
        RefreshText();
        // 시작 시 현재 누적 골드 기준으로 게이지도 맞춰 둠 (애니메이션 없이)
    ...
    {
        // 텍스트 업데이트는 GameManager가 먼저 누적했을 가능성이 높으므로 애니메이션 종료 시점에 갱신
        if (!gameObject.activeInHierarchy) return;
        StartCoroutine(CoFlyAndUpdate(worldPos, amount));
    }
    ...
    private IEnumerator CoFlyAndUpdate(Vector3 worldPos, int amount)
    {
        ...
        if (coinUnit > 0 && amount > 0)
        {
            ...
            coinCount = Mathf.Clamp(coinCount, 1, Mathf.Max(1, maxCoinsPerBurst));
        }
    ...
    {
        int value = gameManager.EarnedMoney;
    ...
    {
        if (gameManager == null) return progress != null ? progress.fillAmount : 0f;
        var stage = gameManager.CurrentStage;
        if (progress == null || stage == null) return 0f;

        int earned = Mathf.Max(0, gameManager.EarnedMoney);
    ...
    {
        var stage = gameManager != null ? gameManager.CurrentStage : null;
        ...
        {
            if (goalSfx != null) soundSource.PlayOneShot(goalSfx);
