...
public class HeartGauge : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        if (gameManager != null)
        {
            gameManager.OnFeverStateChanged += HandleFeverChanged;
            // 현재 상태 반영
            if (gameManager.IsFever) StartHeartbeat(); else StopHeartbeat();
        }
        ...
        // 현재 상태로 즉시 동기화
        displayedHeart = Mathf.Max(0, gameManager != null ? gameManager.CurrentHeart : 0);
        if (progress != null)
    ...
    {
        Customer.OnHeartGained -= HandleHeartGained;
        if (gameManager != null) gameManager.OnFeverStateChanged -= HandleFeverChanged;
        // 심장박동 정지 및 원복
        StopHeartbeat();
    }
    ...
    private IEnumerator CoObserveHeart()
    {
        ...
        {
            int gmHeart = gameManager != null ? Mathf.Max(0, gameManager.CurrentHeart) : displayedHeart;
            if (gmHeart != displayedHeart)
            {
                // 이벤트 없이 변화(감소/증가)가 발생한 경우 부드럽게 보간
                AnimateProgressTo(CalcFillFor(gmHeart));
                displayedHeart = gmHeart;
            }
            yield return null;
        }
    }
    ...
    private void HandleHeartGained(int amount, Customer customer, Vector3 worldPos)
    {
        ...
        StartCoroutine(CoFlyOneHeartAndAnimate(worldPos, amount));
    }

    private float CalcFillFor(int heart)
    {
        // GameManager 또는 Stage가 없으면 기본 1로 나눔(0 division 방지)
        int max = 1;
        if (gameManager != null && gameManager.CurrentStage != null)
        {
            max = Mathf.Max(1, gameManager.CurrentStage.FeverHeartCount);
        }
    ...
    private IEnumerator CoFlyOneHeartAndAnimate(Vector3 worldPos, int amount)
    {
        ...
        if (canvasRect == null || heartIcon == null)
        {
            ...
            yield return null; // 다음 프레임: GameManager.CurrentHeart 갱신 보장
            int finalByGm = gameManager != null ? Mathf.Max(0, gameManager.CurrentHeart) : displayedHeart + Mathf.Max(0, amount);
            AnimateProgressTo(CalcFillFor(finalByGm));
            displayedHeart = finalByGm;
            yield break;
        }
        ...
        // GameManager가 누적을 먼저 갱신하므로, 현재 값을 참조해 최종 fill로 설정
        int finalHeart = gameManager != null ? Mathf.Max(0, gameManager.CurrentHeart) : displayedHeart + Mathf.Max(0, amount);
